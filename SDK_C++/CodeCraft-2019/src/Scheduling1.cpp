...
void lv_scheduling::run(vector<Car>& cars,vector<Road>& roads,vector<Cross>& crosses,vector<Trajectory>& trajectorys)
{
...
//############################################
		...
		ShortestPathDijkstra(netTime,STARTCAR(testCarNum),ENDCAR(testCarNum),tra);	
		tra.setCarId(it->id());
